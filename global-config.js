...
==================================================================
 [global-config.js] - النسخة المعتمدة والمحصنة (إضافة العداد الذكي)
...
function startBootSequence(pageTitle) {
  ...
  // تهيئة العداد عند بدء التشغيل (إذا لم يكن موجوداً)
  window.updateRecordCounter(0); 
}
...
function injectGlobalStyles() {
  ...
  style.textContent = `
    ...
    .record-count-num { color: var(--taxi-gold); font-weight: 900; font-size: 1.1rem; }
    ...
    .sidebar { height: 100vh; width: 230px; position: fixed; z-index: 4000; top: 0; right: -240px; background: rgba(18, 18, 18, 0.75); backdrop-filter: blur(15px); -webkit-backdrop-filter: blur(15px); transition: transform 0.5s cubic-bezier(0.16, 1, 0.3, 1); overflow-y: auto; border-left: 1px solid rgba(255, 255, 255, 0.1); box-shadow: -10px 0 30px rgba(0,0,0,0.5); }
  ...
  const layout = `
    <header class="global-header">
      <div style="display:flex; align-items:center; gap:15px;">
        ...
        <div class="record-counter-capsule">
            <span style="font-size:0.7rem; color:#aaa;">السجلات:</span>
            <span id="recordCount" class="record-count-num">0</span>
        </div>
      </div>
    ...
    <nav id="sideNav" class="sidebar"> 
      <div style="padding:20px; color:var(--taxi-gold); font-weight:bold; text-align:center; font-size:1.1rem; border-bottom:1px solid rgba(255,255,255,0.1);">🚖 نظام إدارة التاكسي </div>
...
// دالة العداد المحصنة (Safe Counter) - لن تعطل النظام أبداً

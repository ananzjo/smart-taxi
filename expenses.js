/* ==================================================================
 ...
 async function fillExpenseDropdowns() {
     try {
         ...
         const carSelect = document.getElementById('f06_car_no');
 ...
 async function saveData() {
     ...
     if (!payload.f06_car_no || !payload.f04_amount) {
         window.showModal("نواقص", "يرجى اختيار السيارة والمبلغ", "warning");
         return;
     }
     const { error } = id 
         ? await _supabase.from('t07_expenses').update(payload).eq('f01_id', id)
         : await _supabase.from('t07_expenses').insert([payload]);
 ...
 function excelFilter() {
     ...
     const filtered = allExpenses.filter(item => {
         return (
             String(item.f06_car_no || "").toLowerCase().includes(val) ||
 ...
 function deleteRecord(id) {
     window.showModal("تأكيد", "هل أنت متأكد من حذف السجل نهائياً؟", "error", async () => {
         const { error } = await _supabase.from('t07_expenses').delete().eq('f01_id', id);

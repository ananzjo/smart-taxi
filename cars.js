/* ==================================================================
 [cars.js] - إدارة أسطول السيارات (متوافق مع الدستور الموحد)
 ================================================================== */
...
function renderTable(list) {
    ...
    list.forEach(car => {
        html += `<tr>
            ...
            <td><span class="badge-${car.f12_is_active === 'نشط' ? 'success' : 'warning'}">${car.f12_is_active}</span></td>
            <td>
...
// [3] الفرز المطور (يدعم العربية والأرقام)
...
// [4] الحفظ والتحديث (الجمع الآلي حسب الدستور)
async function saveData() {
    ...
    if (error) {
        ...
        window.showModal("تم الحفظ", "تم تحديث بيانات السيارة بنجاح ✅", "success");
        resetForm();
        loadData();
    }
}

// [5] الحذف (استخدام المودال العالمي)
function deleteRecord(id) {
    window.openDeleteModal(id, async (targetId) => {
        const { error } = await _supabase.from('t01_cars').delete().eq('f01_id', targetId);
        if (error) {
            window.showModal("خطأ", "لا يمكن حذف السيارة لوجود سجلات مرتبطة بها", "error");
        } else {
            loadData();
        }
    });
